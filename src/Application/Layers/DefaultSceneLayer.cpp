...
void DefaultSceneLayer::_CreateScene()
{
	...
	if (loadScene && std::filesystem::exists("scene.json")) {
		...
		GameObject::Sptr camera = scene->MainCamera->GetGameObject()->SelfRef();
		{
			...
			camera->SetRotation(glm::vec3(4.8f, 0.0f, -0.5f));
		...
		{
			invaderTwo->SetPostion(glm::vec3(-8, 0, 0));
		...
		GameObject::Sptr invaderThree = scene->CreateGameObject("Invader");
		{
			invaderThree->SetPostion(glm::vec3(0, 0, 0));
		...
		GameObject::Sptr invaderFour = scene->CreateGameObject("Invader");
		{
			invaderFour->SetPostion(glm::vec3(8, 0, 0));
		...
		GameObject::Sptr invaderFive = scene->CreateGameObject("Invader");
		{
			invaderFive->SetPostion(glm::vec3(16, 0, 0));
